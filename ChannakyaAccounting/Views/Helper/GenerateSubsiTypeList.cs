...
namespace ChannakyaAccounting.Helper
{
    ...
    public static class GenerateSubsiTypeList
    {
        ...
        public static string GenerateNameForMapSubsi(int ledgerId, int SecondaryID)
        {
            int subsiTbl = 0;
            string name = "";
            using (GenericUnitOfWork uow = new GenericUnitOfWork())
            ...
            if (subsiTbl == 1)
            {
                var list = new Loader.Repository.GenericUnitOfWork().Repository<Loader.Models.Employee>().FindBy(x=>x.EmployeeId==SecondaryID).SingleOrDefault();
                return list.EmployeeName;
            }
            else if (subsiTbl == 2)
            {
                var list = new Loader.Repository.GenericUnitOfWork().Repository<Loader.Models.ApplicationUser>().FindBy(x=>x.EmployeeId==SecondaryID).SingleOrDefault();
                return list.UserName;
            }
            else
            {
                var list = new ChannakyaAccounting.Repository.UnitOfWork.Implementation_Classes.GenericUnitOfWork().Repository<Models.Models.CustIndividual>().FindBy(x=>x.CID==SecondaryID).SingleOrDefault();
             return list.Fname;
            }
            return "";
