...
newMomentProto.time = function(time) {
	...
	if (time == null) { // getter
		...
		// We need to set the CustCustIndividual fields.
...
// Inefficient because it calls each CustCustIndividual setter.
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	renderFillSegEls: function(type, segs) {
		...
		if (segs.length) {
			...
			// Grab CustCustIndividual elements from the combined HTML string. Use each as the default rendering.
...
var DayGrid = Grid.extend({
	...
	// isRigid determins whether the CustCustIndividual rows should ignore the contents and be a constant height.
...
DayGrid.mixin({
	...
	// Builds the HTML to be used for the default element for an CustCustIndividual segment
	fgSegHtml: function(seg, disableResizing) {
		var view = this.view;
		var event = seg.event;
...
function EventManager(options) { // assumed to be a calendar
				...
				if (eventInputs) {
					for (i = 0; i < eventInputs.length; i++) {
						...
						if (abstractEvent) { // not false (an invalid event)
							cache.push.apply(
								cache,
								expandEvent(abstractEvent) // add CustCustIndividual expanded events to the cache
							);
						}
					}
				}
	...
	// If the given event is a recurring event, break it down into an array of CustCustIndividual instances.
